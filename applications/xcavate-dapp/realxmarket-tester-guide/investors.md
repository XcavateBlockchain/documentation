# Investors

1. In exisiting app (version 20/0.1), go to settings where the following page will appear

<figure><img src="../../../.gitbook/assets/image.png" alt="" width="375"><figcaption></figcaption></figure>

2. Click on show mnemonics and enter password

<figure><img src="../../../.gitbook/assets/image (1).png" alt="" width="375"><figcaption></figcaption></figure>

3. Take note of the mnemonics and click export json

<figure><img src="../../../.gitbook/assets/image (2).png" alt="" width="375"><figcaption></figcaption></figure>

3. Enter password to Export json

<figure><img src="../../../.gitbook/assets/image (3).png" alt="" width="375"><figcaption></figcaption></figure>

3. Click save realxmarket.json&#x20;

<figure><img src="../../../.gitbook/assets/image (4).png" alt="" width="375"><figcaption></figcaption></figure>

4. Upon first access the app through Google Play Store, uninstall and upgrade button will appear (please excuse the following screenshot showing Open instead of Upgrade):

<figure><img src="../../../.gitbook/assets/image (40).png" alt="" width="375"><figcaption></figcaption></figure>

5. Click Uninstall and the following screen will appear

<figure><img src="../../../.gitbook/assets/image (41).png" alt="" width="375"><figcaption></figcaption></figure>

6. Click Install and the following screen will appear

<figure><img src="../../../.gitbook/assets/image (42).png" alt="" width="375"><figcaption></figcaption></figure>

7. Click Open and the following view will appear

<figure><img src="../../../.gitbook/assets/image (43).png" alt="" width="375"><figcaption></figcaption></figure>

8. Select Create Account and set a new password as shown below

<figure><img src="../../../.gitbook/assets/image (44).png" alt="" width="375"><figcaption></figcaption></figure>

9. Click Continue and arriving at Enter mnemonics screen

<figure><img src="../../../.gitbook/assets/image (45).png" alt="" width="375"><figcaption></figcaption></figure>

10. Mnemonics can be provided to each person.
11. Enter the Mnemonics provided and click Continue will reach the following screen

<figure><img src="../../../.gitbook/assets/image (46).png" alt="" width="375"><figcaption></figcaption></figure>

12. Click create DID&#x20;

<figure><img src="../../../.gitbook/assets/image (47).png" alt="" width="375"><figcaption></figcaption></figure>

13. The following screen will be shown if you explore how to save mnemonics\


<figure><img src="../../../.gitbook/assets/image (49).png" alt="" width="375"><figcaption></figcaption></figure>

14. Saved the mnemonics and Kilt DID on the screen where your DID has been created. Click Continue to reach 'My account' page

<figure><img src="../../../.gitbook/assets/image (50).png" alt="" width="375"><figcaption></figcaption></figure>
